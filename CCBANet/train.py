...
def parse_arguments():#参数的定义
  ...
  parse.add_argument('--batch_size', type=int, default=4)#初始為8 不能跑
...
def update_config(CONFIG,opt):#opt=parse_arguments() 修改配置
  CONFIG["mode"]== "train"
  CONFIG["dataset"]["name"]=opt.dataset
  CONFIG["train"]["batch_size"]=opt.batch_size#批处理大小
  if opt.use_gpu==True:
    CONFIG["device"]="cuda"
  else:
    CONFIG["device"]="cpu"
  if opt.load_ckpt is not None:#这一块不懂
    CONFIG["resume"]["is_resume"]=True
    CONFIG["resume"]["epoch_start"]=opt.epoch_start
    CONFIG["resume"]["pretrain_model"]=opt.load_ckpt

#Define training excution for each epoch 定义每批的训练执行
...
#Define training evaluation for each epoch 定义每批的训练评估
...
def full_training(config, model, train_dataloader, val_dataloader, optimizer, criteria_loss, criteria_metrics, lr_scheduler):
  ...
  PATH_SAVE_MODEL=config["model_dir"]#"model_dir":CURRENT_DIR+"/output/model", 模型文件夹
  MAX_EPOCHS=config["train"]["max_epochs"]#max_epochs":200,
  EARLY_STOP=config["train"]["early_stop"]#"early_stop":200        
  TRAINING_TIME_OUT = config["train"]["training_time_out"]#"training_time_out":3600*24*3,
  TYPELOSS=config["typeloss"]#"typeloss":"BceDiceLoss",
...
if __name__ == '__main__':
    opt = parse_arguments() #定义参数
    update_config(CONFIG, opt) #修改配置文件中的参数
    print(CONFIG) #输出参数 ok

    train_data, val_data, test_data = LoadDataset(CONFIG) #载入三个数据集的配置 from libraries.dataset import LoadDataset

    #Create dataset and dataloader
    train_dataset, train_dataloader, val_dataset, val_dataloader, _, _ = BuildDatasetAndDataloader(CONFIG, train_data, val_data, test_data)#载入三个数据集详细数据

...
from .modules import ASM, CCM, BAM, SELayer#来自于modules.py文件
...
class DecoderBlock(nn.Module):#解码模块
    def __init__(self, in_channels, out_channels,
        ...
        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)#因为torchvision==0.4.0升级后 需加align_corners=True
...
class SideoutBlock(nn.Module):#不知道什么模块
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1):#super的用法不明白
...
class CCBANet(nn.Module):#网络定义
    def __init__(self, num_classes):
        super(CCBANet, self).__init__()

        resnet = models.resnet34(pretrained=True)#import torchvision.models as models 导入了resnet34的预训练模型
        '''
        resent34源代码
        ...
        self.outconv = nn.Sequential(ConvBlock(64, 32, kernel_size=3, stride=1, padding=1),
                                      nn.Dropout2d(0.1),
                                      nn.Conv2d(32, num_classes, 1))#自定义参数初始化 即将有些层整合成一个部分
    ...
    def forward(self, x):#按论文中例图 反向传播
...
class CCBANetModel(nn.Module): #主函数中执行model = CCBANetModel(CONFIG).to(CONFIG["device"])
  def __init__(self, config):
    super(CCBANetModel,self).__init__()
    self.config=config
    self.num_classes=self.config["model"]["num_classes"] #"model":{"num_classes":1}
    self.net = CCBANet(self.num_classes)

  def forward(self, images):#使用model（）时 自动调用

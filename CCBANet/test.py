...
def parse_arguments():#输出opt
  ...
  parse.add_argument('--load_ckpt', type=str, default='./output/model/model_epoch_zxg.pth')
  
  return parse.parse_args()

def update_config(CONFIG,opt):#更新参数
  CONFIG["mode"] == "test"
...
def epoch_testing(model, test_dataloader, device,criteria_metrics):
  ...
  with torch.no_grad(): # Turn off gradient
    # For each batch
    for step, (images,masks,indexs) in enumerate(test_dataloader):#test_dataloader 那个class对象
      #print(step)
      #print(images)
      #print(masks)
      #print(indexs)
  ...
  score_metrics={#性能参考参数
      #见文档 https://blog.csdn.net/lovemy134611/article/details/108872670
      #https://zhuanlan.zhihu.com/p/78171223?from_voters_page=true&ivk_sa=1024320u
      "recall":_recall,#召回率
       "specificity":_specificity,#特异率
       "precision":_precision,#精确率
       "f1":_F1,
       "f2":_F2,
       "accuracy":_ACC_overall,#准确率
       "iou_poly":_IoU_poly,#息肉iou
       "iou_bg":_IoU_bg,#背景iou
       "iou_mean":_IoU_mean#平均iou
  }
  del images, masks
...
if __name__ == '__main__':
    ...
    update_config(CONFIG, opt)#修改全局参数
    print(CONFIG)

    train_data, val_data, test_data = LoadDataset(CONFIG)#载入测试集
    ...
    if CONFIG["test"]["mask_generating"]==True:
      ...
      for index in np.arange(num_imgs):
        ...
        trans_size= transforms.Resize((h,w),Image.NEAREST)#修改图片大小

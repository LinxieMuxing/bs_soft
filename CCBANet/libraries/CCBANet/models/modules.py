...
class SELayer(nn.Module):#Squeeze&Excite层（顶层）
...
class NonLocalBlock(nn.Module):#Non-Local模块
...
class BAM(nn.Module):#BAM模块（顶层）
    ...
    def forward(self, x, pred):
        ...
        #foregound
        foregound_att= score
        foregound_att=torch.clamp(foregound_att-boundary_att,0,1)#clip又名clamp
        foregound_x= x*foregound_att

        #background
        background_att=1-score
        background_att=torch.clamp(background_att-boundary_att,0,1)

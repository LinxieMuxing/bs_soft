...
#__init__.py中调用了 from .kvasirseg import LoadKvasirSegDataset,KvasirSegDataset
''' 
...
def LoadKvasirSegDataset(config):#读取数据集信息从JSON文件
  data = []
  with open(config['dataset_dir']+'/Kvasir-SEG/kavsir_bboxes.json') as json_file:
    _data = json.load(json_file)#加载json文件
  for k, v in _data.items():#遍历json文件中的key、value
    d=copy.deepcopy(v)#深拷贝value的值 即d['height'] = 529; ...
    d['image'] = k+'.jpg'#d['image'] = cju0qkwl35piu0993l0dewei2.jpg
    data.append(d)#将item插入到date结构数组中
  return data
